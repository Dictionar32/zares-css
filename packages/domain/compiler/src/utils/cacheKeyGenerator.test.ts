...
describe("Cache Key Generator", () => {
  describe("Hash Functions (Group 1)", () => {
    describe("sha256()", () => {
      ...
      it("should return 00000000 for null/undefined", () => {
        ...
        const hash1 = sha256(null)
        // @ts-ignore - testing edge case
        const hash2 = sha256(undefined)
        // These will throw or fail, so we test empty string instead
        const hash3 = sha256("")
        assert.strictEqual(hash3, "00000000")
      })
    ...
    describe("generateFileHash()", () => {
      ...
      it("should be sensitive to whitespace changes", () => {
        ...
        // Note: Should be different due to content change
        assert.notStrictEqual(hash1, hash2)
      })
    })
    ...
    describe("generateVariantHash()", () => {
      ...
      it("should handle duplicate variants (deduplicated by sorting)", () => {
  ...
  describe("Validation (Group 3)", () => {
    describe("validateCacheKey()", () => {
      ...
      it("should reject uppercase (case-insensitive check should pass)", () => {
        // Note: our regex uses case-insensitive flag, so this should pass
        const key = "CSS-COMPILER:A1B2C3D4:E5F6A7B8:C9D0E1F2"
        // This will fail due to prefix case
        assert.strictEqual(validateCacheKey(key), false)
      })
  ...
  describe("Integration Tests (Group 5)", () => {
    it("should support complete caching workflow", () => {
      ...
      assert(parsed !== null)
      assert(parsed.fileHash.length === 8)
      assert(parsed.themeHash.length === 8)
      assert(parsed.variantHash.length === 8)
